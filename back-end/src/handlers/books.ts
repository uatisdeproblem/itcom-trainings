...
class Books extends ResourceController {
  ...
  protected async checkAuthBeforeRequest(): Promise<void> {
    ...
    try {
      ...
      throw new HandledError('Book not found');
    }
  }
  ...
  private async getAllBooksPaginated(startFromBookId?: string): Promise<BookSummary[]> {
    ...
    return (Items as BookSummary[]).map(x => new BookSummary(x));
  }
  ...
  private async getTopBooks(): Promise<Book[]> {
    ...
    return (res.Items as Book[]).map(x => new Book(x));
  }
  ...
  private async rateBook(rating: number): Promise<Book> {
    if (rating < 1 || rating > 5) throw new HandledError('Invalid rating');

    await ddb.put({
      TableName: DDB_TABLES.ratings,
      Item: { bookId: this.resourceId, userId: '@todo', rating }
    });
  ...
  protected async putResource(): Promise<Book> {
    if (this.book.publisherId !== '@todo') throw new HandledError('Unauthorized');
