...
export class BookPage {
  ...
  async ionViewDidEnter(): Promise<void> {
    if (this.bookId !== 'new') {
      await this.loadBookById(this.bookId);
  ...
  async openActions(): Promise<void> {
    ...
    buttons.push({
      ...
      role: 'destructive',
      icon: 'trash',
      handler: (): Promise<void> => this.delete()
    });
    buttons.push({ text: this.t._('COMMON.CANCEL'), role: 'cancel', icon: 'arrow-undo', handler: (): void => {} });
  ...
  private async delete(): Promise<void> {
    ...
    const buttons = [
      ...
      { text: this.t._('COMMON.DELETE'), role: 'destructive', handler: (): Promise<void> => doDelete() }
    ];
  ...
  isMyBookOrNew(): boolean {
    return this.book && !this.book.bookId;
  }
}
...
const sleepNumSeconds = (numSeconds = 1): Promise<void> =>
  new Promise(ok => setTimeout((): void => ok(null), 1000 * numSeconds));
